...
9. **返回类型化对象**。Service 方法返回 Pydantic BaseModel 实例（UserInfo、Department 等），不可变且可哈希，不返回 dict。
10. **日志冒泡到宿主**。使用 NullHandler + SensitiveFilter，propagate=True 让日志自然冒泡给宿主应用的 root logger。
